        ...
        <section class="category-section">
            <div class="container" data-aos="fade-up">
                ...
                <div class="section-header d-flex justify-content-between align-items-center mb-5">
                    <h2>{{ $row->name }}</h2>
                    <div><a href="#" class="more">{{ $row->name }}</a></div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-9">
                            ...
                            <div class="d-lg-flex post-entry-2">
                                ...
                                <div>
                                    ...
                                    <div class="d-flex align-items-center author">
                                        <div class="photo"><img src="{{ Auth::user()->profile->image }}" alt=""
                                                class="img-fluid"></div>
                                        <div class="name">
                        ...
                        <div class="row">
                            {{-- fungsi random untuk mengmabil data secara acak --}}

                            <div class="col-lg-4">
                                @foreach ($row->news->random(1) as $news)
                                    <div class="post-entry-1 border-bottom">
                                        <a href="single-post.html"><img src="{{ $news->image }}" alt=""
                                                class="img-fluid"></a>
                                        <div class="post-meta"><span class="date">{{ $row->name }}</span> <span
                                                class="mx-1">&bullet;</span>
                                        ...
                                        <p class="mb-4 d-block">{!! Str::limit($news->content, 100) !!}</p>
                                    </div>
                                @endforeach
                                ...
                                @foreach ($row->news->random(1) as $news)
                                    <div class="post-entry-1">
                                        <div class="post-meta"><span class="date">{{ $row->name }}</span> <span
                                                class="mx-1">&bullet;</span>
                                            <span>{{ $news->created_at->diffForHumans() }}</span></div>
                                        <h2 class="mb-2"><a href="single-post.html">{{ $news->title }}</a>
                                        </h2>
                                        <span class="author mb-3 d-block">Admin</span>
                                    </div>
                                @endforeach
                            ...
                            @foreach ($row->news->random(1) as $news)
                                <div class="col-lg-8">
                                    <div class="post-entry-1">
                                        ...
                                        <span class="author mb-3 d-block">Admin</span>
                                        <p class="mb-4 d-block">{!! Str::limit($news->content, 200) !!}</p>
                                    </div>
                                </div>
                            @endforeach
                    ...
                    <div class="col-md-3">
                        ...
                        @foreach ($sideNews as $news)
                            <div class="post-entry-1 border-bottom">
                                <div class="post-meta"><span class="date">{{ $row->name }}</span> <span
                                        class="mx-1">&bullet;</span>
                                    {{-- diffForHumans() berfungsi menampilakan waktu dalam bentuk seperti "1 Hours ago" atau "2 Days ago" --}}
                                    {{-- format ('d F Y') berfungsi menampilkan waktu dalam bentuk seperti "12 July 2021" --}}
                                    <span>{{ $news->created_at->diffForHumans() }}</span>
                                </div>
                                <h2 class="mb-2"><a href="single-post.html">
                                        {{-- limit karakter --}}
                                        {{ Str::limit($news->title, 50) }}</a></h2>
                                <span class="author mb-3 d-block">Admin</span>
                            </div>
                        @endforeach
